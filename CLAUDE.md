...
## Looking for the Remy dev tunnel?

It is not here any more. `src/dev/` and the headless mode it served moved into
`@madewithremy/sandbox` in September 2026, where the dev tunnel is a second bin of the
command-and-control server that spawns it — see that package's `src/devTunnel/`. Laptop
development was retired at the same time; every Remy dev session runs in a hosted box.

What is left in this repo is the v1 MindStudio product: routing local AI providers to MindStudio
Cloud, and local editing of custom interfaces and scripts. That is what the guide above covers.
